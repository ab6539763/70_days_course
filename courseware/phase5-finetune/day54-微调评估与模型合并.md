...
> **培训阶段**: 第五阶段 微调与部署 | **第 9 周** | **学习时长**: 约 6-8 小时  
> **今日关键词**: 评估、BLEU、ROUGE、模型合并、GGUF
...
```python
# 使用 LLaMA-Factory 评估
llamafactory-cli eval day54/eval_config.yaml
```

#### LoRA 合并

```bash
llamafactory-cli export     --model_name_or_path Qwen/Qwen2.5-1.5B-Instruct     --adapter_name_or_path saves/qwen-lora     --export_dir models/qwen-finetuned     --export_size 2     --export_device cpu
```
...
- 完成今天的实操项目和课后作业
- 确保代码可运行并提交 GitHub
- Git 提交：`git commit -m "Day 54: 微调评估与模型合并"`
- 预习 Day 55 内容
...
*课程讲义 · 零基础大模型应用开发 70 天培训 · Day 54 · 第五阶段 微调与部署*
...
回顾 Day 53 到 Day 54 的知识串联，绘制思维导图。将今天学的知识与前三阶段（Python、RAG、Agent）的知识点关联起来。
...
# 运行今日代码
python day54/main.py

# 运行测试
python -m pytest day54/tests/ -v

# Git 工作流
git add day54/
git commit -m "Day 54: 完成今日学习"
