...
> **今日关键词**: 数据集、指令格式、Alpaca、ShareGPT、数据清洗
...
def clean_dataset(data):
    cleaned = []
    for item in data:
        if len(item["instruction"]) < 5: continue
        if len(item["output"]) < 2: continue
        if item["output"] == item["instruction"]: continue
        cleaned.append(item)
    return cleaned
```
...
- 完成今天的实操项目和课后作业
- 确保代码可运行并提交 GitHub
- Git 提交：`git commit -m "Day 52: 数据集构建"`
- 预习 Day 53 内容
...
*课程讲义 · 零基础大模型应用开发 70 天培训 · Day 52 · 第五阶段 微调与部署*
...
回顾 Day 51 到 Day 52 的知识串联，绘制思维导图。将今天学的知识与前三阶段（Python、RAG、Agent）的知识点关联起来。
...
- **简历关键词**: 数据集、指令格式、Alpaca、ShareGPT、数据清洗
- **面试高频题**: 参见 FAQ 部分
