...
面试八股：Transformer、注意力机制、RAG原理、Agent架构
...
**Q: 解释 Self-Attention 的计算过程**

A: 
1. 输入 X 通过三个权重矩阵生成 Q、K、V
2. Attention(Q,K,V) = softmax(QK^T / √d_k) × V
3. 多头注意力：h 组独立注意力，拼接后线性变换
...
- Temperature Sampling：按温度调整概率分布后采样
- Top-p (Nucleus)：从累积概率达 p 的最小 token 集合中采样

#### RAG 相关

**Q: RAG 的完整流程？**

A: 文档加载 → 分块 → 向量化 → 存储 → 查询向量化 → 相似度检索 → 重排序 → 拼接上下文 → LLM 生成
...
A: ReAct 是 Prompt 驱动的文本格式（Thought/Action/Observation），Function Calling 是模型原生 API 能力，直接输出结构化 JSON。现代 Agent 多用 Function Calling。

**Q: 如何防止 Agent 无限循环？**

A: max_iterations 限制、死循环检测（连续相同 Action）、Token 预算、超时机制、熔断器。
...
- 完成今天的实操项目和课后作业
- 确保代码可运行并提交 GitHub
- Git 提交：`git commit -m "Day 67: 面试专题一技术八股"`
- 预习 Day 68 内容
...
*课程讲义 · 零基础大模型应用开发 70 天培训 · Day 67 · 第六阶段 毕业与求职*
...
回顾 Day 66 到 Day 67 的知识串联，绘制思维导图。将今天学的知识与前三阶段（Python、RAG、Agent）的知识点关联起来。
